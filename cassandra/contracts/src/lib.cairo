...
mod PredictionMarket {
    use super::{Market, UserStake, ContractAddress};
    use starknet::get_caller_address;
    use starknet::get_block_timestamp;
    use starknet::storage::StorageMap;
    use starknet::storage::StorageMapAccess;
    ...
    impl PredictionMarketImpl of super::IPredictionMarket<ContractState> {
        ...
        fn resolve_market(ref self: ContractState, market_id: u32, outcome: bool) {
            ...
            let mut resolved_market = market;
            resolved_market.is_resolved = true;
            resolved_market.outcome = outcome;
            self.markets.write(market_id, resolved_market);
        ...
        fn stake_no(ref self: ContractState, market_id: u32, amount: u256) {
            ...
            let mut user_stake = self.user_stakes.read((caller, market_id));
            user_stake.no_amount += amount;
            self.user_stakes.write((caller, market_id), user_stake);

            let mut updated_market = market;
            updated_market.no_staked += amount;
            updated_market.total_staked += amount;
            self.markets.write(market_id, updated_market);
        ...
        fn claim_winnings(ref self: ContractState, market_id: u32) {
            ...
            let winning_amount = if market.outcome {
                if user_stake.yes_amount > 0 {
                    (user_stake.yes_amount * market.total_staked) / market.yes_staked
                } else {
                    0
                }
            } else {
                if user_stake.no_amount > 0 {
                    (user_stake.no_amount * market.total_staked) / market.no_staked
                } else {
                    0
                }
            };
