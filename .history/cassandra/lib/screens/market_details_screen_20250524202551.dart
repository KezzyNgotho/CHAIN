...
class _MarketDetailsScreenState extends State<MarketDetailsScreen> {
  ...
  Future<void> _loadComments() async {
    // TODO: Implement comment loading from your backend
    setState(() {
      comments = [
        {
          'id': 1,
          'user': '0x123...abc',
          'text': 'This is a great prediction market!',
          'likes': 5,
          'isOwner': false,
        },
        {
          'id': 2,
          'user': '0x456...def',
          'text': 'I think the outcome will be different.',
          'likes': 2,
          'isOwner': true,
        },
      ];
  ...
  void _handleUnlike(int commentId) {
    setState(() {
      final comment = comments.firstWhere((c) => c['id'] == commentId);
      comment['likes'] = (comment['likes'] as int) - 1;
    });
  ...
  void _handleDelete(int commentId) {
    setState(() {
                      ...
                      ListView.builder(
                        ...
                        itemBuilder: (context, index) {
                          ...
                          return CommentWidget(
                            ...
                            marketId: int.parse(widget.marketId),
                            isOwner: comment['isOwner'],
                            onLike: () => _handleLike(comment['id']),
                            onUnlike: () => _handleUnlike(comment['id']),
