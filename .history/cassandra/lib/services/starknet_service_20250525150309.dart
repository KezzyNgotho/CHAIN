  ...
  Future<List<Map<String, dynamic>>> getMarkets() async {
    try {
      final count = await predictionMarketContract.call(
        selector:
            PredictionMarketABI.getSelector(PredictionMarketABI.getMarketCount),
        calldata: PredictionMarketABI.prepareCalldata(
            PredictionMarketABI.getMarketCount, []),
      );
      ...
      for (var i = 0; i < count[0].toInt(); i++) {
        final market = await predictionMarketContract.call(
          selector:
              PredictionMarketABI.getSelector(PredictionMarketABI.getMarket),
          calldata: PredictionMarketABI.prepareCalldata(
              PredictionMarketABI.getMarket, [i]),
        );
        markets.add({
          'id': i,
    ...
    try {
      final market = await predictionMarketContract.call(
        selector:
            PredictionMarketABI.getSelector(PredictionMarketABI.getMarket),
        calldata: PredictionMarketABI.prepareCalldata(
            PredictionMarketABI.getMarket, [marketId]),
      );
      return {
        'id': marketId,
        'question': market[0].toString(),
  ...
  Future<List<Map<String, dynamic>>> getComments(int marketId) async {
    try {
      final commentCount = await predictionMarketContract.call(
        selector: PredictionMarketABI.getSelector(
            PredictionMarketABI.getCommentCount),
        calldata: PredictionMarketABI.prepareCalldata(
            PredictionMarketABI.getCommentCount, [marketId]),
      );
      ...
      for (var i = 0; i < commentCount[0].toInt(); i++) {
        final comment = await predictionMarketContract.call(
          selector:
              PredictionMarketABI.getSelector(PredictionMarketABI.getComment),
          calldata: PredictionMarketABI.prepareCalldata(
              PredictionMarketABI.getComment, [marketId, i]),
        );
        comments.add({
          'id': i,
