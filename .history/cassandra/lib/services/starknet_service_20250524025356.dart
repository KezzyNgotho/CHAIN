...
class StarkNetService {
  ...
  Future<bool> connectWallet(String privateKey) async {
    try {
      ...
      final signer = Signer(privateKey);
  ...
  Future<List<Map<String, dynamic>>> getMarkets() async {
    try {
      final result = await contract.call('get_markets', []);
  ...
  Future<List<Map<String, dynamic>>> getComments(int marketId) async {
    try {
      final result =
          await contract.call('get_comments', [Felt.fromInt(marketId)]);
  ...
  Future<void> addComment(int marketId, String text) async {
    try {
      await contract.invoke(
          'add_comment', [Felt.fromInt(marketId), Felt.fromString(text)]);
    } catch (e) {
  ...
  Future<void> deleteComment(int marketId, int commentId) async {
    try {
      await contract.invoke(
          'delete_comment', [Felt.fromInt(marketId), Felt.fromInt(commentId)]);
    } catch (e) {
  ...
  Future<void> likeComment(int marketId, int commentId) async {
    try {
      await contract.invoke(
          'like_comment', [Felt.fromInt(marketId), Felt.fromInt(commentId)]);
    } catch (e) {
  ...
  Future<void> unlikeComment(int marketId, int commentId) async {
    try {
      await contract.invoke(
          'unlike_comment', [Felt.fromInt(marketId), Felt.fromInt(commentId)]);
    } catch (e) {
