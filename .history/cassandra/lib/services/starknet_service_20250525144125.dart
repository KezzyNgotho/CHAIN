...
class StarkNetService {
  ...
  Future<List<Map<String, dynamic>>> getMarkets() async {
    try {
      ...
      for (var i = 0; i < count[0].toInt(); i++) {
        final market = await predictionMarketContract.call('getMarket', [Felt.fromInt(i)]);
        markets.add({
          'id': i,
  ...
  Future<Map<String, dynamic>> getMarket(int marketId) async {
    try {
      final market = await predictionMarketContract.call('getMarket', [Felt.fromInt(marketId)]);
      return {
        'id': marketId,
        'question': market[0].toString(),
        'endTime': market[1].toInt(),
  ...
  Future<String> placeBet(int marketId, bool isYes, int amount) async {
    try {
      final tx = await predictionMarketContract.execute(
        'placeBet',
        [Felt.fromInt(marketId), Felt.fromInt(isYes ? 1 : 0), Felt.fromInt(amount)],
      );
      return tx.hash.toString();
    } catch (e) {
  ...
  Future<List<Map<String, dynamic>>> getComments(int marketId) async {
    try {
      final commentCount = await predictionMarketContract.call('getCommentCount', [Felt.fromInt(marketId)]);
      final comments = <Map<String, dynamic>>[];

      for (var i = 0; i < commentCount[0].toInt(); i++) {
        final comment = await predictionMarketContract.call('getComment', [Felt.fromInt(marketId), Felt.fromInt(i)]);
        comments.add({
          'id': i,
