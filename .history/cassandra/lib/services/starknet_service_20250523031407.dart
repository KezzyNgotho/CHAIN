...
import 'package:starknet/src/utils/constants.dart';
...
class StarkNetService {
  ...
  StarkNetService() {
    ...
    account = Account(
      provider: provider,
      address: Felt.fromHexString('0x...'), // Your account address
      privateKey:
          Felt.fromHexString('0x...'), // Your private key (keep this secure!)
    );
  ...
  Future<Map<String, dynamic>> getPredictionDetails(String predictionId) async {
    try {
      final result = await contract.call(
        ...
        calldata: [Felt.fromHexString(predictionId)],
      );
