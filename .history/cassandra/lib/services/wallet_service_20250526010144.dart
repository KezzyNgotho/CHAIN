...
class WalletService extends ChangeNotifier {
  ...
  Future<List<String>> _requestArgentXConnection() async {
    try {
      // First check if Argent X is installed
      final uri = Uri.parse('argentx://connect');
      final canLaunch = await canLaunchUrl(uri);

      if (!canLaunch) {
        // If Argent X is not installed, provide installation instructions
        final installUri = Uri.parse('https://www.argent.xyz/argent-x/');
        if (await canLaunchUrl(installUri)) {
          await launchUrl(installUri);
        }
        throw Exception(
            'Argent X wallet is not installed. Please install it from https://www.argent.xyz/argent-x/');
      }
      ...
      // In production, you would implement proper wallet connection handling
      return [
