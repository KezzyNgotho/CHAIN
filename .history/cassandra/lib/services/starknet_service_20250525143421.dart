...
class StarkNetService {
  ...
  Future<List<Map<String, dynamic>>> getMarkets() async {
    try {
      final count =
          await predictionMarketContract.functions['getMarketCount'].call();
      final markets = <Map<String, dynamic>>[];

      for (var i = 0; i < count[0].toInt(); i++) {
        final market =
            await predictionMarketContract.functions['getMarket'].call(
          marketId: Felt.fromInt(i),
        );
        markets.add({
          'id': i,
  ...
  Future<Map<String, dynamic>> getMarket(int marketId) async {
    try {
      final market = await predictionMarketContract.functions['getMarket'].call(
        marketId: Felt.fromInt(marketId),
      );
      return {
        'id': marketId,
        'question': market[0].toString(),
  ...
  Future<String> createMarket(String question, int endTime) async {
    try {
      final tx =
          await predictionMarketContract.functions['createMarket'].invoke(
        question: Felt.fromString(question),
        endTime: Felt.fromInt(endTime),
        maxFee: Felt.fromInt(100000000000000), // 0.0001 ETH
      );
  ...
  Future<String> placeBet(int marketId, bool isYes, int amount) async {
    try {
      final tx = await predictionMarketContract.functions['placeBet'].invoke(
        marketId: Felt.fromInt(marketId),
        isYes: Felt.fromInt(isYes ? 1 : 0),
        amount: Felt.fromInt(amount),
        maxFee: Felt.fromInt(100000000000000), // 0.0001 ETH
      );
      return tx.transactionHash.toString();
    } catch (e) {
  ...
  Future<String> resolveMarket(int marketId, bool outcome) async {
    try {
      final tx =
          await predictionMarketContract.functions['resolveMarket'].invoke(
        marketId: Felt.fromInt(marketId),
        outcome: Felt.fromInt(outcome ? 1 : 0),
        maxFee: Felt.fromInt(100000000000000), // 0.0001 ETH
      );
      return tx.transactionHash.toString();
    } catch (e) {
  ...
  Future<String> claimWinnings(int marketId) async {
    try {
      final tx =
          await predictionMarketContract.functions['claimWinnings'].invoke(
        marketId: Felt.fromInt(marketId),
        maxFee: Felt.fromInt(100000000000000), // 0.0001 ETH
      );
      return tx.transactionHash.toString();
  ...
  Future<List<Map<String, dynamic>>> getComments(int marketId) async {
    try {
      final commentCount =
          await predictionMarketContract.functions['getCommentCount'].call(
        marketId: Felt.fromInt(marketId),
      );
      ...
      for (var i = 0; i < commentCount[0].toInt(); i++) {
        final comment =
            await predictionMarketContract.functions['getComment'].call(
          marketId: Felt.fromInt(marketId),
          commentId: Felt.fromInt(i),
        );
        comments.add({
          'id': i,
  ...
  Future<String> addComment(int marketId, String text) async {
    try {
      final tx = await predictionMarketContract.functions['addComment'].invoke(
        marketId: Felt.fromInt(marketId),
        text: Felt.fromString(text),
        maxFee: Felt.fromInt(100000000000000), // 0.0001 ETH
      );
      return tx.transactionHash.toString();
    } catch (e) {
  ...
  Future<String> deleteComment(int marketId, int commentId) async {
    try {
      final tx =
          await predictionMarketContract.functions['deleteComment'].invoke(
        marketId: Felt.fromInt(marketId),
        commentId: Felt.fromInt(commentId),
        maxFee: Felt.fromInt(100000000000000), // 0.0001 ETH
      );
      return tx.transactionHash.toString();
    } catch (e) {
  ...
  Future<String> likeComment(int marketId, int commentId) async {
    try {
      final tx = await predictionMarketContract.functions['likeComment'].invoke(
        marketId: Felt.fromInt(marketId),
        commentId: Felt.fromInt(commentId),
        maxFee: Felt.fromInt(100000000000000), // 0.0001 ETH
      );
  ...
  Future<String> unlikeComment(int marketId, int commentId) async {
    try {
      final tx =
          await predictionMarketContract.functions['unlikeComment'].invoke(
        marketId: Felt.fromInt(marketId),
        commentId: Felt.fromInt(commentId),
        maxFee: Felt.fromInt(100000000000000), // 0.0001 ETH
      );
