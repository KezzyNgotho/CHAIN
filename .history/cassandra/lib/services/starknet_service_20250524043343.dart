...
class StarkNetService {
  ...
  final Map<String, dynamic> _mockProposal = {
    ...
    'start_time': DateTime.now().millisecondsSinceEpoch.toString(),
    'end_time':
        DateTime.now().add(Duration(days: 7)).millisecondsSinceEpoch.toString(),
  ...
  Future<String> stakeTokens(String amount, int duration) async {
    ...
    _mockStake['end_time'] = DateTime.now()
    ...
    // Update staking pool
    final amountWei =
        BigInt.parse(_mockStakingPool['total_staked']) + BigInt.parse(amount);
  ...
  Future<String> unstake() async {
    ...
    // Update staking pool
    final amountWei = BigInt.parse(_mockStakingPool['total_staked']) -
        BigInt.parse(_mockStake['amount']);
    _mockStakingPool['total_staked'] = amountWei.toString();
  ...
  Future<String> createStakingProposal(
    ...
    _mockProposal['start_time'] =
        DateTime.now().millisecondsSinceEpoch.toString();
    _mockProposal['end_time'] =
        DateTime.now().add(Duration(days: 7)).millisecondsSinceEpoch.toString();
  ...
  Future<String> voteOnStakingProposal(String proposalId, bool vote) async {
    ...
    // Update mock proposal data
    if (vote) {
      final yesVotes = BigInt.parse(_mockProposal['yes_votes']) +
          BigInt.from(1000000000000000000);
      _mockProposal['yes_votes'] = yesVotes.toString();
    } else {
      final noVotes = BigInt.parse(_mockProposal['no_votes']) +
          BigInt.from(1000000000000000000);
  ...
  Future<String> stake(String predictionId, bool isYes, String amount) async {
    ...
    if (isYes) {
      final yesAmount =
          BigInt.parse(market['yesAmount']) + BigInt.parse(amount);
  ...
  Future<Map<String, dynamic>> getPredictionDetails(String predictionId) async {
    ...
    return {
      ...
      'totalStaked':
          (BigInt.parse(market['yesAmount']) + BigInt.parse(market['noAmount']))
              .toString(),
