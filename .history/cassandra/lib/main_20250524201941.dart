...
import 'screens/widgets/comment_section.dart';
import 'screens/widgets/comment_modal.dart';
...
class ExplorePage extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    return const Center(child: Text('Explore Coming Soon'));
  }
}
...
class _MarketsPageState extends State<MarketsPage>
  ...
  void _navigateToMarketDetails(Map<String, String> market) {
    Navigator.push(
      context,
      MaterialPageRoute(
          ...
          children: [
            ...
            Expanded(
              child: TabBarView(
                controller: _tabController,
                children: [
                  _buildMarketsList(theme, isActive: true),
                  _buildMarketsList(theme, isActive: false),
                ],
              ),
            ),
          ],
  ...
  Widget _buildMarketsList(ThemeData theme, {required bool isActive}) {
    if (_isLoading) {
    ...
    final markets = [
      ...
      },
      {
        ...
        'timeLeft': '15d',
        'category': 'Tech',
        'description': 'Apple AR glasses release prediction',
    ...
    final filteredMarkets = _selectedCategory == 'All'
        ? markets
        : markets.where((m) => m['category'] == _selectedCategory).toList();
    ...
    filteredMarkets.sort((a, b) {
      switch (_sortBy) {
        ...
        case 'Most Staked':
          return int.parse(b['totalStaked']!.replaceAll(RegExp(r'[^0-9]'), ''))
              .compareTo(int.parse(
                  a['totalStaked']!.replaceAll(RegExp(r'[^0-9]'), '')));
        case 'Ending Soon':
          return int.parse(a['timeLeft']!.replaceAll(RegExp(r'[^0-9]'), ''))
              .compareTo(
                  int.parse(b['timeLeft']!.replaceAll(RegExp(r'[^0-9]'), '')));
        case 'Trending':
          return int.parse(b['participants']!.replaceAll(RegExp(r'[^0-9]'), ''))
              .compareTo(int.parse(
                  a['participants']!.replaceAll(RegExp(r'[^0-9]'), '')));
        default:
          return 0;
      }
    });
