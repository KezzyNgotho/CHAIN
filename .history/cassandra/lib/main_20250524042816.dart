...
class _StakeScreenState extends State<StakeScreen> {
  ...
  Future<void> _updateExpectedReturn() async {
    ...
    try {
      final returnAmount = await _starknetService.getExpectedReturn(
        'prediction_id', // Replace with actual prediction ID
        widget.stakeOn == 'YES',
        _amountController.text,
      );
      setState(() => _expectedReturn = returnAmount);
    } catch (e) {
  ...
  Future<void> _handleStake() async {
    ...
    try {
      final txHash = await _starknetService.stake(
        'prediction_id', // Replace with actual prediction ID
        widget.stakeOn == 'YES',
        _amountController.text,
      );
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content:
                Text('Stake successful! TX: ${txHash.substring(0, 10)}...'),
