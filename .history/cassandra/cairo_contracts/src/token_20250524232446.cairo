...
pub mod Token {
    ...
    impl TokenImpl of super::IToken<ContractState> {
        fn mint(ref self: ContractState, to: ContractAddress, amount: u256) {
            ...
            let mut balance = self.balances.read((to, 0));
            balance += amount;
            self.balances.write((to, 0), balance);
        ...
        fn transfer(ref self: ContractState, to: ContractAddress, amount: u256) {
            let caller = get_caller_address();
            let mut from_balance = self.balances.read((caller, 0));
            assert(from_balance >= amount, 'Insufficient balance');

            from_balance -= amount;
            self.balances.write((caller, 0), from_balance);

            let mut to_balance = self.balances.read((to, 0));
            to_balance += amount;
            self.balances.write((to, 0), to_balance);
