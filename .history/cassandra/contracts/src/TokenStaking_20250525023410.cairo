...
mod TokenStaking {
    ...
    impl TokenStakingImpl of ITokenStaking<ContractState> {
        ...
        fn unstake(ref self: ContractState) {
            ...
            self.emit(Unstaked { user: caller, amount: stake_amount, rewards: rewards });
        }
        ...
        fn vote_on_staking_proposal(ref self: ContractState, proposal_id: u64, vote: bool) {
            ...
            let stake_amount = stake.amount;
            if vote {
                proposal.yes_votes += stake_amount;
            } else {
                proposal.no_votes += stake_amount;
            }
            
            self.staking_votes.write((proposal_id, caller), (true, vote));
            self.staking_proposals.write(proposal_id, proposal);
            
            self.emit(StakingVoteCast { proposal_id: proposal_id, voter: caller, vote: vote, stake_amount: stake_amount });
        }
