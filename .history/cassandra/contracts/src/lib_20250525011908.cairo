...
mod PredictionMarket {
    ...
    impl PredictionMarketImpl of super::IPredictionMarket<ContractState> {
        ...
        fn add_comment(ref self: ContractState, market_id: u32, text: felt252) {
            ...
            array::ArrayTrait::append(ref user_comments, market_id);
        ...
        fn delete_comment(ref self: ContractState, market_id: u32, comment_id: u32) {
            ...
            // Remove comment by setting to a zeroed comment (Cairo doesn't support deletion)
            let zero_comment = Comment {
                id: comment_id,
                user: ContractAddress::zero(),
                text: 0,
                timestamp: 0,
                likes: 0,
            };
            self.comments.write((market_id, comment_id), zero_comment);
            // Remove from user's comments (optional: not implemented for brevity)
        }
        ...
        fn like_comment(ref self: ContractState, market_id: u32, comment_id: u32) {
            ...
            let mut i = 0;
            loop {
                if i >= array::ArrayTrait::len(likes) {
                    break;
                }
                if array::ArrayTrait::at(likes, i) == caller {
                    has_liked = true;
                    break;
                }
                i += 1;
            ...
            if !has_liked {
                ...
                array::ArrayTrait::append(ref new_likes, caller);
        ...
        fn unlike_comment(ref self: ContractState, market_id: u32, comment_id: u32) {
            ...
            if has_liked {
                comment.likes -= 1;
                self.comments.write((market_id, comment_id), comment);
                // Remove from likes (not implemented for brevity)
            }
        }
        ...
        fn get_comments(self: @ContractState, market_id: u32) -> Array<Comment> {
            ...
            loop {
                ...
                let comment = self.comments.read((market_id, i));
                array::ArrayTrait::append(ref arr, comment);
                i += 1;
            }
            arr
        }
