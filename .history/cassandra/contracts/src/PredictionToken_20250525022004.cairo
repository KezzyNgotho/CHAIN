...
mod PredictionToken {
    ...
    #[external(v0)]
    impl PredictionTokenImpl of super::PredictionToken<ContractState> {
        ...
        fn balanceOf(self: @ContractState, account: felt252) -> (balance: u256) {
            let (balance) = self.balances.read(account);
            return (balance);
        }

        fn allowance(self: @ContractState, owner: felt252, spender: felt252) -> (amount: u256) {
            let (amount) = self.allowances.read(owner, spender);
            return (amount);
        }
    }
} 