...
async def test_prediction_market():
    try:
        ...
        try:
            # Create the call
            call = contract.functions["create_market"].prepare(
                question=question,
                category=category,
                end_time=end_time
            )
            ...
            tx = await account.execute(call, max_fee=100000000000000)  # 0.0001 ETH
        ...
        try:
            # Create the call
            call = contract.functions["place_bet"].prepare(
                market_id=0,
