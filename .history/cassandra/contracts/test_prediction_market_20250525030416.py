...
async def test_prediction_market():
    try:
        ...
        try:
            ...
            print(f"Transaction hash: {hex(tx.hash)}")
        ...
        try:
            market = await contract.functions["get_market"].call(market_id=0)
            print(f"Market ID: {market.id}")
            print(f"Question: {market.question}")
            print(f"Category: {market.category}")
            print(f"End Time: {market.end_time}")
            print(f"Total Staked: {market.total_staked}")
        except Exception as e:
        ...
        print("\nPlacing a stake...")
        try:
            stake_tx = await contract.functions["stake"].invoke(
                market_id=0,
                amount=1000000000000000000,  # 1 ETH
                is_yes=1,  # 1 for true in Cairo
                max_fee=100000000000000  # 0.0001 ETH
            )
            print("Stake transaction sent! Waiting for acceptance...")
            await stake_tx.wait_for_acceptance()
            print("Stake placed successfully!")
            print(f"Transaction hash: {hex(stake_tx.hash)}")
        except Exception as e:
            print(f"Error placing stake: {str(e)}")
