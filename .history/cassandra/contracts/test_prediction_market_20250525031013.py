...
from starknet_py.hash.starknet_hash import starknet_keccak
...
# Function selectors (computed from function names)
CREATE_MARKET_SELECTOR = starknet_keccak("createMarket".encode("ascii")).to_bytes(32, "big")
PLACE_BET_SELECTOR = starknet_keccak("placeBet".encode("ascii")).to_bytes(32, "big")
GET_MARKET_SELECTOR = starknet_keccak("getMarket".encode("ascii")).to_bytes(32, "big")
    ...
    try:
        ...
        try:
            ...
            call = Call(
                ...
                selector=int.from_bytes(CREATE_MARKET_SELECTOR, "big"),
                calldata=[question, category, end_time]
            )
            
            # Execute the transaction
            tx = await account.execute(calls=[call], max_fee=100000000000000)  # 0.0001 ETH
        ...
        try:
            call = Call(
                to_addr=CONTRACT_ADDRESS,
                selector=int.from_bytes(GET_MARKET_SELECTOR, "big"),
                calldata=[0]  # market_id
            )
        ...
        try:
            ...
            call = Call(
                to_addr=CONTRACT_ADDRESS,
                selector=int.from_bytes(PLACE_BET_SELECTOR, "big"),
                calldata=[0, 1000000000000000000, 1]  # market_id, amount, is_yes
            )
